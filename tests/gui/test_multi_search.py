...
class FakeClient:
    ...
    def __init__(self, pages: dict, raw_counts: dict | None = None):
        self.pages = pages
    ...
    async def search(self, tags, page, limit):
        ...
        if isinstance(batch, Exception):
            raise batch
        self.last_search_raw_count = self.raw_counts.get(page, len(batch))
        return batch
...
def test_raw_api_count_not_parsed_count_determines_exhaustion():
    """Danbooru may drop posts without file_url; raw JSON size is the signal."""
    c = FakeClient({1: [P(0), P(1)]}, raw_counts={1: 4})
    collected, _, exhausted, _ = asyncio.run(
        fetch_site_page(
            c, "cat", 1, 4, set(), set(), set(), backfill_delay=0, backfill=False,
        )
    )
...
def test_paged_mode_skips_backfill():
    seen = {(1, 0), (1, 1)}
    page1 = [P(i, site_id=1) for i in range(4)]
    page2 = [P(i, site_id=1) for i in range(10, 14)]
    c = FakeClient({1: page1, 2: page2})
    collected, last_page, _, _ = asyncio.run(
        fetch_site_page(
            c, "cat", 1, 4, set(), set(), seen, backfill_delay=0, backfill=False,
        )
    )
    assert len(collected) == 2
    assert last_page == 1
