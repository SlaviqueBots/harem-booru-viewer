...
class BooruApp(QMainWindow):
    def __init__(self) -> None:
        ...
        # HAREM_FORK: Link Bridge toolbar actions.
        from ..harem.controller import HaremBridgeController

        self._harem_bridge = HaremBridgeController(self)
    ...
    def _setup_ui(self) -> None:
        ...
        self._browse_back_btn = QToolButton()
        self._browse_back_btn.setIcon(
            self.style().standardIcon(QStyle.StandardPixmap.SP_ArrowBack)
        )
        self._browse_back_btn.setToolTip("Browse back (Alt+Left)")
        self._browse_back_btn.setFixedSize(28, 24)
        self._browse_back_btn.clicked.connect(self._search_ctrl.browse_back)
        top.addWidget(self._browse_back_btn)

        self._browse_forward_btn = QToolButton()
        self._browse_forward_btn.setIcon(
            self.style().standardIcon(QStyle.StandardPixmap.SP_ArrowForward)
        )
        self._browse_forward_btn.setToolTip("Browse forward (Alt+Right)")
        self._browse_forward_btn.setFixedSize(28, 24)
        self._browse_forward_btn.clicked.connect(self._search_ctrl.browse_forward)
        top.addWidget(self._browse_forward_btn)
        ...
        self._preview.harem_bridge_requested.connect(self._harem_bridge.send_action)  # HAREM_FORK
        ...
        QShortcut(QKeySequence("Alt+Left"), self, self._search_ctrl.browse_back)
        QShortcut(QKeySequence("Alt+Right"), self, self._search_ctrl.browse_forward)
        QShortcut(QKeySequence("Ctrl+C"), self, self._copy_file_to_clipboard)

        self.update_browse_nav_buttons()
    ...
    def update_browse_nav_buttons(self) -> None:
        """Enable browse back/forward when history allows (fixed-size, no layout shift)."""
        if not hasattr(self, "_stack"):
            return
        on_browse = self._stack.currentIndex() == 0
        hist = self._search_ctrl._browse_history
        self._browse_back_btn.setEnabled(on_browse and hist.can_go_back())
        self._browse_forward_btn.setEnabled(on_browse and hist.can_go_forward())
    ...
    def _apply_scroll_speed(self) -> None:
        """HAREM_FORK: load scroll multiplier only — never touch scrollbar steps."""
        from ..harem.scroll import parse_scroll_multiplier, set_scroll_multiplier

        set_scroll_multiplier(
            parse_scroll_multiplier(self._db.get_setting("scroll_speed_multiplier"))
        )
