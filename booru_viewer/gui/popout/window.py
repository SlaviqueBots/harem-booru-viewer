...
class FullscreenPreview(QMainWindow):
    ...
    closed = Signal()
    harem_bridge_requested = Signal(str)
    ...
    def _on_context_menu(self, pos) -> None:
        ...
        bridge_actions: dict[int, str] = {}
        if self._bookmark_btn.isVisible():
            from ...harem.menu import append_bridge_actions

            bridge_actions = append_bridge_actions(menu)
            menu.addSeparator()
        ...
        elif id(action) in bridge_actions:
            self.harem_bridge_requested.emit(bridge_actions[id(action)])
