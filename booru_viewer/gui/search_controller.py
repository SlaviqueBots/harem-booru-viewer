...
from .browse_history import BrowseHistory, BrowseSnapshot, copy_search_state, page_is_at_end
...
async def fetch_site_page(
    ...
    *api_exhausted* means the API returned a short batch (no more
    ...
    api_exhausted = _search_batch_exhausted(client, batch, limit)
    if not backfill:
    ...
    for _ in range(9):
        ...
        for k in drops:
            drops[k] += batch_drops[k]
        collected.extend(filtered)
        if _search_batch_exhausted(client, batch, limit):
            api_exhausted = True
    return collected, last_page, api_exhausted, drops


def _search_batch_exhausted(client, batch: list, limit: int) -> bool:
    """True when the API has no further pages (uses raw JSON count when set)."""
    raw = getattr(client, "last_search_raw_count", 0)
    if raw:
        return raw < limit
...
class SearchController:
    ...
    def __init__(self, app: BooruApp) -> None:
        ...
        self._restoring_browse = False
        self._last_api_exhausted = False
    ...
    def reset(self) -> None:
        ...
        self._search.page_api_exhausted.clear()
        self._browse_history = BrowseHistory()
        self._cached_names = None
        self._bookmarked_ids = None
        self._saved_ids = None
        self._app.update_browse_nav_buttons()
    ...
    def browse_back(self) -> None:
        snap = self._browse_history.go_back(self._capture_browse_snapshot())
        if snap is not None:
            self._restore_browse_snapshot(snap)
        self._app.update_browse_nav_buttons()

    def browse_forward(self) -> None:
        snap = self._browse_history.go_forward(self._capture_browse_snapshot())
        if snap is not None:
            self._restore_browse_snapshot(snap)
        self._app.update_browse_nav_buttons()

    def _push_browse_history(self) -> None:
        if self._restoring_browse:
            return
        if not self._current_tags and not self._app._posts:
            return
        self._browse_history.push_back(self._capture_browse_snapshot())

    def _capture_browse_snapshot(self) -> BrowseSnapshot:
        grid = self._app._grid
        selected = grid.selected_index if grid.selected_index >= 0 else -1
        return BrowseSnapshot(
            search_state=copy_search_state(self._search),
            current_page=self._current_page,
            current_tags=self._current_tags,
            current_rating=self._current_rating,
            min_score=self._min_score,
            media_filter=self._app._media_filter.currentText(),
            search_bar_text=self._app._search_bar.text(),
            posts=list(self._app._posts),
            scroll_value=grid.scroll_position(),
            selected_index=selected,
            infinite_scroll=self._infinite_scroll,
        )

    def _restore_browse_snapshot(self, snap: BrowseSnapshot) -> None:
        self._restoring_browse = True
        try:
            self._current_tags = snap.current_tags
            self._current_rating = snap.current_rating
            self._min_score = snap.min_score
            self._current_page = snap.current_page
            self._search = copy_search_state(snap.search_state)
            self._app._search_bar.set_text(snap.search_bar_text)
            self._app._score_spin.setValue(snap.min_score)
            idx = self._app._media_filter.findText(snap.media_filter)
            if idx >= 0:
                self._app._media_filter.setCurrentIndex(idx)
            self._app._page_spin.setValue(snap.current_page)
            self._app._page_label.setText(f"Page {snap.current_page}")
            self._app._preview.clear()
            self._loading = False
            self._display_posts(
                list(snap.posts),
                [],
                scroll_to_top=False,
                scroll_value=snap.scroll_value,
        ...
        async def _search():
            ...
            try:
                results = await asyncio.gather(
                    *(
                        fetch_site_page(
                            c, t, page, limit, bl_tags, bl_posts, seen,
                            backfill=self._infinite_scroll,
                        )
                ...
                api_exhausted = bool(oks) and not errors and all(r[2] for r in oks)
                self._last_api_exhausted = api_exhausted
                log.debug(
    ...
    def on_search_done(self, posts: list, errors: list | None = None) -> None:
        self._display_posts(posts, errors or [], scroll_to_top=True, update_cache=True)
    ...
    def _display_posts(
        ...
        *,
        ...
        if update_cache:
            ...
            ss.page_api_exhausted[self._current_page] = self._last_api_exhausted
            if not self._infinite_scroll and len(ss.page_cache) > 10:
                oldest = min(ss.page_cache.keys())
                del ss.page_cache[oldest]
                ss.page_api_exhausted.pop(oldest, None)
        limit = self._app._db.get_setting_int("page_size") or 40
        api_exhausted = ss.page_api_exhausted.get(self._current_page, self._last_api_exhausted)
        at_end = page_is_at_end(
            api_exhausted=api_exhausted,
            result_count=len(posts),
            page_size=limit,
        )
        log.debug(
            f"_display_posts: displayed_count={len(posts)} limit={limit} "
            f"api_exhausted={api_exhausted} at_end={at_end}"
        )
        ...
        if scroll_to_top:
            ...
            QTimer.singleShot(50, lambda v=scroll_value: self._app._grid.set_scroll_position(v))
        from PySide6.QtCore import QTimer
        ...
        elif selected_index >= 0 and selected_index < len(posts):
            self._app._grid._select(selected_index)
